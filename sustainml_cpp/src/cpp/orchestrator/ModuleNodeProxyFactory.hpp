...
struct ModuleNodeProxyFactory
{
    static void make_node_proxy(
            ...
            std::unique_ptr<ModuleNodeProxy> &node_proxy)
    {
        switch (id)
        {
            case NodeID::ID_TASK_ENCODER:
            {
                node_proxy = std::make_unique<TaskEncoderNodeProxy>(part, db);
                break;
            }
            case NodeID::ID_MACHINE_LEARNING:
            {
                node_proxy = std::make_unique<MLModelProviderNodeProxy>(part, db);
                break;
            }
            case NodeID::ID_HARDWARE_RESOURCES:
            {
                node_proxy = std::make_unique<HardwareResourcesProviderNodeProxy>(part, db);
                break;
            }
            case NodeID::ID_CARBON_FOOTPRINT:
            {
                node_proxy = std::make_unique<CarbonFootprintProviderNodeProxy>(part, db);
                break;
            }
            default:
            {
