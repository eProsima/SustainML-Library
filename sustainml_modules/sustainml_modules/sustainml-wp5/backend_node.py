...
import signal
import sys
from orchestrator_node import orchestrator_node, utility
...
def results():
    ...
    app_req = orchestrator.get_results(utility.node_id.APP_REQUIREMENTS.value, last_task_id)
    metadata = orchestrator.get_results(utility.node_id.ML_MODEL_METADATA.value, last_task_id)
    constraints = orchestrator.get_results(utility.node_id.HW_CONSTRAINTS.value, last_task_id)
    model = orchestrator.get_results(utility.node_id.ML_MODEL_PROVIDER.value, last_task_id)
    hardware = orchestrator.get_results(utility.node_id.HW_PROVIDER.value, last_task_id)
    carbontracker = orchestrator.get_results(utility.node_id.CARBONTRACKER.value, last_task_id)
    json = {f'{utility.string_node(utility.node_id.APP_REQUIREMENTS.value)}': f'{app_req}',
            f'{utility.string_node(utility.node_id.ML_MODEL_METADATA.value)}': f'{metadata}',
            f'{utility.string_node(utility.node_id.HW_CONSTRAINTS.value)}': f'{constraints}',
            f'{utility.string_node(utility.node_id.ML_MODEL_PROVIDER.value)}': f'{model}',
            f'{utility.string_node(utility.node_id.HW_PROVIDER.value)}': f'{hardware}',
            f'{utility.string_node(utility.node_id.CARBONTRACKER.value)}': f'{carbontracker}'}
...
def results_args():
    ...
    return jsonify({f'{utility.string_node(node_id)}': f'{orchestrator.get_results(node_id, task_id)}'}), 200
